...
class VisualServoyer(object):
    ...
    def camera_colour_callback(self, img):
        ...
        try:
            ...
            self.show_image(orig_frame, "Quokka Servo 2")
...
def main():
    ...
    cyan = np.array([[90, 90, 0],
                     [110, 110, 20]])
    black = np.array([[0, 0, 0],
                    [20, 20, 20]])
    purple = np.array([[80, 10, 80],
                      [120, 30, 120]])

    # Create a list of all the marker hsv colour ranges
    vs.masks = [cyan, black, purple]
    vs.labels = ["cyan", "black", "purple"]

    # Initialise the node and the publishers and subscribers
    rospy.init_node("visual_servoyer2", anonymous=True)

    # vs.publisher = rospy.Publisher("/landmarks", Landmarks_Msg, queue_size=1)
    vs.publisher = rospy.Publisher("/servoing_results2", Twist, queue_size=1)
